...
- **APP summary view:** a **single unified filter bar** (`SF` state: plan, stages, chars, regions, missions, comparator) drives every visual together - headline stats, lifecycle donut, demographic bar chart, and the **map**. Filters: Current/Past/All plan; multi-select lifecycle stage (Access, and Success split into Continuation/Attainment/Completion, plus Progression - see `stageKey()`); multi-select region and mission-group chips; a searchable multi-select demographic dropdown; a searchable **comparator-set builder** (pick specific institutions - when active it overrides the region+mission filters and shows only those institutions); and a "Reset all filters" button. `summaryFilteredTargets()` is the single source of truth; `buildSummary()` re-renders all visuals on any change.
